...
namespace DungeonSlime.Scenes
{
    public class TitleScene : Scene
    {
        ...
        public override void Initialize()
        {
            ...
            // Initalize the offset of the background pattern at zero
            _backgroundOffset = Vector2.Zero;

            // Set the background pattern destination to fill the entire 
            // screen background
            _backgroundDestination = Core.GraphicsDevice.PresentationParameters.Bounds;

            // Set GUM UI Title screen 
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // Update the offsets for the background pattern wrapping it so that
            // it scrolls down and to the right
            float offset = _scrollSpeed * (float)gameTime.ElapsedGameTime.TotalSeconds;
            _backgroundOffset.X -= offset;
            _backgroundOffset.Y -= offset;

            // Ensure that the offsets fo not go beyond the texture bounds
            // so it is a seamless wrap
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Core.SpriteBatch.DrawString(_font, PRESS_ENTER_TEXT, _pressEnterPos, Color.White, 0.0f, _pressEnterOrigin, 1.5f, SpriteEffects.None, 0.0f);
